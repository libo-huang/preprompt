...
def build_continual_dataloader(args):
    ...
    return dataloader, dataloader_per_cls, class_mask, target_task_map  # 以task分割的loader, 以cls分割的loader, 以task分割的cls id, 所有cls对应的task id
...
def split_single_dataset(dataset_train, dataset_val, args):
    ...
    for i in range(args.num_tasks):
        ...
        for k in range(len(dataset_train.targets)):  # 类标签在 scope 的样本 index 存到 train_split_indices
            if int(dataset_train.targets[k]) in scope:
                train_split_indices.append(k)

        for h in range(len(dataset_val.targets)):  # 类标签在 scope 的样本 index 存到 test_split_indices
    ...
    for i in range(len(mask)):
        ...
        for cls_id in single_task_labels:
            ...
            for k in range(len(dataset_train.targets)):
                if args.dataset.startswith('5-datasets'):
                    label_k = int(dataset_train.targets[k] + dataset_train.target_transform.nb_classes)
                else:
                    label_k = int(dataset_train.targets[k])
                if label_k == cls_id:
                    train_split_indices.append(k)
            ...
            for h in range(len(dataset_val.targets)):
                if args.dataset.startswith('5-datasets'):
                    label_h = int(dataset_val.targets[h] + dataset_val.target_transform.nb_classes)
                else:
                    label_h = int(dataset_val.targets[h])
                if label_h == cls_id:
                    test_split_indices.append(h)
