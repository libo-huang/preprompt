...
class Attention(nn.Module):
    ...
    def forward(self, x, *args):
        ...
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)  # [3 24 12 197 64]. 为什么 requires_grad: False？ (trainers/tii_trainer.py L43)
        q, k, v = qkv.unbind(0)  # make torchscript happy (cannot use tensor as tuple). x.unbind(0)会沿着x的第0维进行切片

        attn = (q @ k.transpose(-2, -1)) * self.scale  # 批量矩阵乘法
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        x = (attn @ v).transpose(1, 2).reshape(B, N, C)  # C=self.num_heads*C // self.num_heads 合并多头注意力机制
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.attn = attn_layer(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)  # 768, 12, True, 0.0, 0.0
        self.ls1 = LayerScale(dim, init_values=init_values) if init_values else nn.Identity()  # nn.Identity()
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path1 = DropPath(drop_path) if drop_path > 0. else nn.Identity()  # nn.Identity()

        self.norm2 = norm_layer(dim)  # LayerNorm((768,)
        self.mlp = Mlp(in_features=dim, hidden_features=int(dim * mlp_ratio), act_layer=act_layer, drop=drop)  # 768--3072--768
        self.ls2 = LayerScale(dim, init_values=init_values) if init_values else nn.Identity()  # nn.Identity()
        self.drop_path2 = DropPath(drop_path) if drop_path > 0. else nn.Identity()  # nn.Identity()

    def forward(self, x, prompt=None):
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x), prompt)))  # [24, 197, 768]
        x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x))))  # [24, 197, 768]
        return x
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.class_token = class_token  # True
        self.num_prefix_tokens = 1 if class_token else 0  # 1
        self.no_embed_class = no_embed_class  # False
        ...
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim)) if class_token else None  # [1,1,768]
        ...
        self.head_type = head_type  # 'token'
        self.use_prompt_mask = use_prompt_mask  # True

        self.use_g_prompt = use_g_prompt  # False
        ...
        num_g_prompt = len(self.g_prompt_layer_idx) if self.g_prompt_layer_idx is not None else 0  # 0
        self.use_prefix_tune_for_g_prompt = use_prefix_tune_for_g_prompt  # False

        self.use_e_prompt = use_e_prompt  # True
        self.e_prompt_layer_idx = e_prompt_layer_idx  # [0,1,2,3,4]
        num_e_prompt = len(self.e_prompt_layer_idx) if self.e_prompt_layer_idx is not None else 0  # 5
        self.use_prefix_tune_for_e_prompt = use_prefix_tune_for_e_prompt  # Ture
        ...
        if use_e_prompt and e_prompt_layer_idx is not None:
            self.e_prompt = EPrompt(length=prompt_length, embed_dim=embed_dim, embedding_key=embedding_key,  # 5, 768, 'cls',
                                    prompt_init=prompt_init,  # 'uniform'
                                    prompt_pool=prompt_pool, prompt_key=prompt_key, pool_size=pool_size, top_k=top_k,  # True, False, 10, 1,
                                    batchwise_prompt=batchwise_prompt,  # False,
                                    prompt_key_init=prompt_key_init, num_layers=num_e_prompt,  # 'uniform', 5,
                                    use_prefix_tune_for_e_prompt=use_prefix_tune_for_e_prompt,  # True,
                                    num_heads=num_heads, same_key_value=same_key_value)  # 12, False,
        ...
        if self.prompt_pool:
            if not self.use_prefix_tune_for_g_prompt:
                self.total_prompt_len += g_prompt_length * len(self.g_prompt_layer_idx)  # 0
            if not self.use_prefix_tune_for_e_prompt:
                self.total_prompt_len += prompt_length * top_k * len(self.e_prompt_layer_idx)

        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]  # stochastic depth decay rule. [0.0, 0.0, ..., 0.0]
        self.blocks = nn.Sequential(*[
            block_fn(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, init_values=init_values,  # 768, 12, 4.0, True, None, 
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer,  # 0.0, 0.0, 0.0, LayerNorm, GELU
                attn_layer=attn_layer)  # attention.PreT_Attention
            for i in range(depth)])  # 12
        self.norm = norm_layer(embed_dim) if not use_fc_norm else nn.Identity()  # LayerNorm((768,)
    ...
    def init_weights(self, mode=''):
        ...
        head_bias = -math.log(self.num_classes) if 'nlhb' in mode else 0.  # 0.0
    ...
    @torch.jit.ignore()
    def load_pretrained(self, checkpoint_path, prefix=''):  # self, '/home/libo/.cache/torch/hub/checkpoints/ViT-B_16.npz', ''
    ...
    def forward_features(self, x, task_id=-1, prompt_id=None, prompt_weight=None, train=False, prompt_momentum=0):
        ...
        if self.cls_token is not None:
            x = torch.cat((self.cls_token.expand(x.shape[0], -1, -1), x), dim=1)  # [24 197 768]

        x = self.pos_drop(x + self.pos_embed)  # [24 197 768]

        if self.grad_checkpointing and not torch.jit.is_scripting():
            x = checkpoint_seq(self.blocks, x)
        else:
            if self.use_g_prompt or self.use_e_prompt:  # general prompt, expert prompt; 训练和测试时没有用general prompt
                if self.use_prompt_mask and train:  # 训练时根据task id构造prompt mask
                    ...
                    if end > self.e_prompt.pool_size:
                        prompt_mask = None
                    if task_id == 0:  # 初始任务不存在历史prompt 所以momentum=0
                        prompt_momentum = 0
                else:
                    prompt_mask = None
                g_prompt_counter = -1
                ...
                e_prompt = res['batched_prompt']  # [5, 24, 2, 5, 12, 64]
                ...
                for i, block in enumerate(self.blocks):
                    if i in self.g_prompt_layer_idx:
                        ...
                        if self.use_prefix_tune_for_e_prompt:
                            ...
                            x = block(x, prompt=e_prompt[e_prompt_counter])  # 0:[24,197,768]; 1:[24,197,768]; 2:[24,197,768]; 3:[24,197,768]; 4:[24,197,768]
                        else:
                        ...
                        x = block(x)  # 5-11:[24,197,768]; 虽然用了prompt，但是qkv的矩阵乘法消去了中间维度202.
            else:
                x = self.blocks(x)

                res = dict()

        x = self.norm(x)  # [24, 197, 768]
        res['x'] = x 

        return res

    def forward_head(self, res, pre_logits: bool = False):  # [24 197 768], False
        x = res['x']  # [24, 197, 768]
        ...
        x = self.mlp(x)  # original_model mlp=[768, 1536, 768]; model mlp=Identity()
        x = self.fc_norm(x)  # [24,768]

        res['logits'] = self.head(x)  # [24,100]
...
def get_init_weights_vit(mode='jax', head_bias: float = 0.):  # '', 0.0
    if 'jax' in mode:
...
def _create_vision_transformer(variant, pretrained=False, **kwargs):
    ...
    model = build_model_with_cfg(
        ...
        pretrained_custom_load='npz' in pretrained_cfg['url'],  # True
        **kwargs)
    return model
